...
import { ILogger, ILoggerFactory } from "../../../services/logger/abstraction";
...
export class ViewSingIeIssue extends React.Component<IViewSingIeIssueProps, IViewSingIeIssue> {
  ...
  constructor(props: IViewSingIeIssueProps, context: React.ContextType<typeof AppServiceContext>) {
    ...
    const loggerFactory = context.get<ILoggerFactory>(AppService.LoggerFactory);
    this.logger = loggerFactory.create("ViewIssue");
  ...
  async componentDidMount() {
    this.logger.info(`componentDidMount with issue ${this.getIssueId()}`)
  ...
  async loadIssue() {
    ...
    try {      
      ...
      this.setState({ issue });
      this.logger.debug("issue loaded", issue);
    } catch (e) {      
  ...
  async loadComments() {
    ...
    try {      
      ...
      this.logger.debug("comments loaded", comments);
      this.setState({ comments })
    } catch (e) {
      this.logger.error("Error loading comments", e);
    } finally {
  ...
  async addComment(text: string) {
    ...
    this.logger.debug("Comment added", { issueId: this.getIssueId(), text })
    await this.loadComments();
  }
  ...
  async changeIssueStatus(state: IssueStatus) {
    ...
    this.logger.info("Issue status changed", { id, state })
